...
- 選擇 X 軸、Y 軸與分類欄位。
- 支援長條圖、折線圖、圓餅圖、散佈圖、堆疊長條圖、時間序列圖、面積圖、水平長條圖、雷達圖、矩形式樹狀圖、漏斗圖、儀表圖、箱型圖、熱力圖、K 線圖、桑基圖、旭日圖、關係圖、主題河流圖與日曆熱力圖。
...
- 箱型圖：X 軸選分類名稱，Y 軸選數字欄位，同一分類需有多筆數值才能看出分布。
- 熱力圖：X 軸與 Y 分類選兩個分類欄位，數值欄位會彙總成交叉格強度。
- K 線圖：日期欄位作為 X 軸，並選擇開盤、最高、最低、收盤四個數字欄位。
- 桑基圖、關係圖：來源與目標選文字或分類欄位，權重選數字欄位。
- 旭日圖：父層與子層選分類欄位，數值欄位用於計算區塊大小。
- 主題河流圖：日期欄位作為 X 軸，分類欄位作為系列，數值欄位作為河流寬度。
- 日曆熱力圖：日期欄位作為 X 軸，數值欄位會依日期彙總成日曆色塊。
